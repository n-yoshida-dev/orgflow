...
- internal organization 管理者が実行する操作
  ...
  - membership role の付与 / 剥奪 / 一覧取得

特に、`membership role` の付与・剥奪・一覧取得は tenant 管理者ではなく、**対象 internal organization の管理権限を持つ internal organization 管理者** が実行する。

また、`membership role 一覧取得` の事前条件は、状態変更を伴う操作ではないため、  
...
- approval policy / approval route / membership role は、いずれも internal organization 単位の業務運用に近く、同じスコープで揃えた方が理解しやすい
- 一覧取得は作成・更新・削除とは違い、存在確認と閲覧権限の成立が中心であり、状態変更系と同じ事前条件に寄せると不自然になる

### 不採用案

- 案A: membership role の付与 / 剥奪 / 一覧取得も tenant 管理者に寄せる
  - tenant 全体管理と部門単位の業務権限管理が混ざる
  - 課内・部門内での権限委譲を説明しにくい
- 案B: membership role 一覧取得の事前条件を、付与 / 剥奪と同じ「対象 membership が存在すること」に寄せる
  - 一覧取得の対象は role 付与結果の集合であり、状態変更系の単一対象存在確認と同じ粒度で扱うと不自然になる
...
- `membership role 一覧取得` の「対象」を internal organization とみなすため、OpenAPI では membership 個別取得ではなく、internal organization 配下の role 付与結果一覧として表現する方が自然になる

### 見直す条件

- internal organization 管理者が、自部門配下だけでなく複数部門を横断して membership role を管理する必要が出た場合
