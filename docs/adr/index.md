...
各 ADR には、決定、その理由、検討した他の案、受け入れる制約、見直す条件を残します。
...
| # | 決定 | Status | 日付 |
|---|---|---|---|
| [001](./ADR-001-題材に申請承認アプリを採用する.md) | 題材として申請・承認フローを持つ業務アプリを採用する | Accepted | 2026-03-19 |
...
| [004](./ADR-004-状態遷移を明示的な状態モデルで表現する.md) | request の状態を `request_status` と遷移ルールで明示的に表現する | Accepted | 2026-03-19 |
| [005](./ADR-005-requestは申請者と組織を直接参照する.md) | request は申請者 user と internal organization を直接参照する | Accepted | 2026-03-24 |
| [006](./ADR-006-applied_approval_routeの保存構造.md) | applied_approval_route を3層構造で保存する（結論は 009 と同一） | **Superseded by 009** | 2026-03-24 |
| [007](./ADR-007-requestを申請版の単位として扱う.md) | request を「申請版」の単位とし、系列と版番号で識別する | Accepted | 2026-03-24 |
...
| [010](./ADR-010-同一承認段は1人の承認で完了する.md) | 同一承認段は誰か1人の承認で完了する | Accepted | 2026-03-24 |
| [011](./ADR-011-audit_logはFKを置かず操作時点の証跡を保持する.md) | audit_log の操作対象は `target_type` / `target_id` で持ち FK は置かない | Accepted | 2026-03-25 |
| [012](./ADR-012-approval_policyがapproval_routeを参照する.md) | `approval_policy` が `approval_route_id` を外部キーとして持つ | Accepted | 2026-03-25 |
...
| [017](./ADR-017-OpenAPIをAPI契約として先に整備する.md) | OpenAPI を API 契約として v1 の重要 endpoint から先に管理する | Accepted | 2026-04-19 |
...
| [030](./ADR-030-監査ログは業務処理と同一トランザクションで書く.md) | 監査ログは業務処理と同一トランザクションで書く | Accepted | 2026-08-21 |
| [031](./ADR-031-HTTPステータスはクライアントの次の行動で選ぶ.md) | HTTP ステータスは「クライアントが次に何をすべきか」で選ぶ | Accepted | 2026-08-22 |
| [032](./ADR-032-エラー応答の文言はハンドラが例外種別ごとに決める.md) | エラー応答の文言はハンドラが例外種別ごとに決め、例外のメッセージは応答に含めない | Accepted | 2026-08-22 |
| [033](./ADR-033-単体テストはmockで隔離しDBを伴う検証はTestcontainersのPostgreSQLで行う.md) | 単体テストは mock で隔離し、DB を伴う検証は Testcontainers の PostgreSQL で行う | Accepted | 2026-08-23 |
| [034](./ADR-034-seedはdb-seedに分離しFlywayのlocationsで環境ごとに切り替える.md) | seed は `db/seed/` に分離し、Flyway の locations で環境ごとに適用を切り替える | Accepted | 2026-08-25 |
| [035](./ADR-035-状態列はenum型ではなくtextとCHECK制約で表現する.md) | 状態列は PostgreSQL の enum 型ではなく text と CHECK 制約で表現する | Accepted | 2026-08-25 |
| [036](./ADR-036-mainへの変更はPRとsquashマージのみで取り込む.md) | main への変更は PR と squash マージのみで取り込む | Accepted | 2026-08-25 |
...
- エラー応答: 031 / 032
- 監査ログ: 002 / 011 / 030
- 業務フローと request のスキーマ表現: 035
- ローカル開発基盤: 018 / 019 / 029 / 034
- テスト: 033
- 開発フロー（ブランチ運用）: 036
