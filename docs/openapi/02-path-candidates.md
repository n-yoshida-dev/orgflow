...
### 2-3. 自分が所属するtenant一覧取得

| path              | 暫定method | pathの種類  | 主リソース | 気になる点                                                                                         |
| ----------------- | ---------- | ----------- | ---------- | -------------------------------------------------------------------------------------------------- |
| /me/tenants       | GET        | subresource | tenant     | `/me` は利用者自身を表す便宜的なpathであり、主リソースを user と見るか tenant と見るかがやや揺れる |
| /tenants          | GET        | collection  | tenant     | tenant全体の一覧取得やtenant管理APIに見える                                                        |
| /users/me/tenants | GET        | subresource | tenant     | 意味は明確だが、Phase 1 のAPIとしてはやや長い                                                      |

- 第一候補: GET /me/tenants
- 理由: 取得したいのは任意userのtenant一覧ではなく、認証済みuser自身が所属しているtenant一覧であるため。クライアントからuserIdを渡さず、JWTの `sub` からuserIdを取得する設計と対応しやすい。
- 主要4xx:
  - 401: 未ログイン、JWT不正、JWTの `sub` がUUIDとして不正なケース

- 未確定な点: 将来的にtenantの表示順、tenant role、利用停止状態などを返す必要が出た場合、response schemaを拡張するか、別APIに分けるか。

### 2-4. current tenant切り替え
