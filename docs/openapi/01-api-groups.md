...
この群は、利用者による `login`、所属tenant一覧取得、`current tenant 切り替え` などの認証・コンテキスト操作を扱う。
ログイン、tenant選択前の所属tenant確認、現在利用中tenantの切り替えなどは含めるが、user管理やtenant管理などの管理操作は含めない。
...
- `login` は未ログインの利用者が呼ぶ。
- `自分が所属するtenant一覧取得` はログイン済みの利用者が呼ぶ。
...
| ---------------------------- | -------------------------------- | -------------------------------- |
| login                        | 認証情報                         | -                                |
| 自分が所属するtenant一覧取得 | tenant                           | `current_tenant_id` は使用しない |
...
- 自分が所属するtenant一覧取得を `/me/tenants` として置くのが自然か
